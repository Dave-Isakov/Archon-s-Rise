...
- **Comments are sparse.** The spec is the commentary. Comment only where the code alone would mislead â€” a non-obvious ordering requirement, or a deliberate deviation from a nearby pattern. Do not restate the spec in the code. This overrides the surrounding files' existing heavy comment density; do not match it.
...
**Interfaces:**
- Consumes: nothing.
- Produces: `public static List<T> ShopRules.PickUnique<T>(IReadOnlyList<T> pool, int count, Func<int,int> rng) where T : class` â€” returns at most `count` distinct non-null entries in draw order; fewer when the distinct pool is smaller; empty for a null/empty pool or `count <= 0`. `rng(exclusiveMax)` must return `[0, exclusiveMax)`.

- [x] **Step 1: Build the two CLI harnesses**

Write `<scratchpad>/Runner.cs` â€” a reflection runner for `[Test]` methods:
...
Write `<scratchpad>/CompileCheck.ps1` â€” compile-checks Unity assemblies while the editor holds the lock:
...
Never invoke `mcs.bat` (it breaks on the spaces in `C:\Program Files\...`), and never run the bare `Runner.exe` (the mcs build links Mono's mscorlib â€” it must run under `mono.exe`).
...
Expected: FAIL to compile â€” `error CS2001: Source file '...ShopRules.cs' could not be found.`
...
- [x] **Step 1: Add the field to `TownsSO`**
...
- [x] **Step 2: Replace the with-replacement sampling in `OfferCardChoice`**
...
- [x] **Step 4: Compile-check the changed assemblies**
...
- [x] **Step 5: Re-run the Task 1 tests to confirm nothing regressed**
...
- Produces: `TownActionSnapshot(PlaceType placeType, bool conquered, int guardiansRemaining, int influence, int healCost, int crystalCost, bool sellsCards, int cardCost, bool anyUnitAffordable, bool visitCanAct, bool hasMenu)` â€” the two new parameters sit between `crystalCost` and `anyUnitAffordable`. Task 4's `TownToken.BuildActions` must pass them by name.
...
In the same file, change `ActionSpent_ServicesLockButMenuStaysOpen` so it covers the Cards slot too â€” replace its first line:
    ...
    public void AllowedServices_NoPlaceTypeGrantsCards()
    {
        foreach (PlaceType type in System.Enum.GetValues(typeof(PlaceType)))
            Assert.IsFalse(PlaceRules.AllowedServices(type).HasFlag(PlaceService.Cards),
                $"{type} must not gate Cards â€” the card list does");
    }
```
...
Expected: FAIL to compile â€” `error CS1729: 'TownActionSnapshot' does not contain a constructor that takes 11 arguments`.
...
- [x] **Step 4: Gate the Cards slot on the list**
...
Leave the `PlaceService.Cards` enum member in place â€” removing it would renumber the flags.
...
Expected: every line `PASS`, ending `30 passed, 0 failed` (8 `PlaceRulesTests` + 22 `PlaceActionRulesTests`). `Assembly-CSharp` will not compile until Task 4 updates `TownToken` â€” that is expected here and is not a reason to change this task.
...
- Produces: `public void TownToken.BuyCards()` â€” spends `townSO.cardLevel` influence, commits the visit action, then opens the picker. Both UI routes call it and nothing else.

Both files are `MonoBehaviour`s, so the gate is the Roslyn compile-check plus Task 5's play test.

- [x] **Step 1: Feed the snapshot's new fields**
...
- [x] **Step 2: Add `BuyCards` and dispatch to it**
...
- [x] **Step 3: Replace the `CardButton` stub**
...
public class CardButton : TownButtons
{
    ...
    public override void UpdateButtonText()
    {
        ...
        buttonText.text =
            $"{IconMarkup.Tag(IconConcept.Card)} Cards â€” {IconMarkup.Cost(IconConcept.Influence, _town.townSO.cardLevel)}";
...
- [x] **Step 4: Compile-check every affected assembly**
...
- [x] **Step 5: Re-run every pure test**
...
This task is the user's, done in the Unity editor. Do not hand-edit the `.asset` YAML â€” hand these instructions over and wait.

- [ ] **Step 1: Hand the user the authoring instructions**

> The code is in. Unity will recompile when you focus the editor â€” the Console should be clean.
>
...
> 3. Check **Card Level** on the same asset â€” that is now the influence price per
...
- [ ] **Step 2: Play test â€” the fan route**
...
5. Re-entering the place the same turn: every service slot is now dimmed â€” the visit's action was spent by the purchase.

- [ ] **Step 3: Play test â€” the skip, the canvas route, and the non-sellers**

1. Buy again on a later turn and press **Skip** on the picker. Influence stays spent â€” no refund â€” and the picker closes cleanly.
...
3. Stand on a place with an empty `purchasableCards` list â€” including a **Castle**, which used to show a locked "Cards (soon)" slot. Neither the fan nor the town menu shows a Cards entry at all.
...
If either `.meta` path does not exist yet, drop it from the `git add` â€” Unity writes it on the next refresh, and it can ride along with a later commit.
...
| Behavior | Task 5 Steps 2â€“3 | every numbered item confirmed by the user |
