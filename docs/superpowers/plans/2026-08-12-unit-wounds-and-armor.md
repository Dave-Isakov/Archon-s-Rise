...
**Spec coverage:** §1 principle → Task 8. §3 armor + soak math → Tasks 4, 8. §4 picker → Tasks 9, 10. §5 unit state → Tasks 5, 6. §6.1–6.3 healing → Tasks 11, 12. §6.4 town guard → Tasks 1, 2, 3, 6. §6.5 disband shows wound state → Task 6 Step 8. §7 army HUD → Task 13. §8 persistence → Task 7. §10 balance → Tasks 14, 15. §11 tests → Tasks 1, 2, 6, 7, 8. §12 manual edits → Task 14. §13 docs → Task 15.

**One gap found during review and closed:** §6.5's disband-panel wound display had no task. `DisbandPanel.cs:44` renders the label from script, so it is a real one-line change; it is now Task 6 Step 8 with the actual code rather than a note to figure it out later.

**Two things the plan cannot verify and hands to the human instead:** every task touching a `MonoBehaviour` (3, 5, 9, 10, 11, 12, 13) ends in a compile check plus explicit in-editor verification steps, because the editor lock blocks batch-mode `-runTests` and these types are not reachable from the mcs harness. That is a real limit of the setup, not a shortcut — the pure rules underneath them (Tasks 1, 2, 6, 7, 8) carry the actual test coverage.
